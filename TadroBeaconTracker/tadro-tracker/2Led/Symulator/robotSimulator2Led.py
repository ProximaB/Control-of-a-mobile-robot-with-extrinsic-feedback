...
from robot import Robot2Led, Robot2LedBicycle
...
class robotSimulationEnv2LedBicycle:
    ...
    def draw_robot_position(self, frame):
        ...
        led1_pos, led2_pos, time, robot_center, heading, theta, length, diamater, axle_len = robot.unpackImg(true_height, CFG.AREA_HEIGHT_REAL, true_width, CFG.AREA_WIDTH_REAL)
        #leds
        ...
        radious = length; axes = (radious,radious)
        angle=heading*180/np.pi
        startAngle=-10; endAngle=10#0-180def
        color=(255, 0, 0)
        cv.ellipse(area_frame, rnd, axes, angle, startAngle, endAngle, color, CFG.ROBOT_THICKNESS)
        for i in range(1, radious):
            ax=tuple([a - i for a in axes])
            cv.ellipse(area_frame, rnd, ax, angle, startAngle/i, endAngle/i, color, CFG.ROBOT_THICKNESS)
        #pole robocze robota
        mh = m+h
        mw = m+w
    ...
    def simulation_keys_KLIO(self):
        ...
        while True:
            ...
            log_print(f'S:{S} T:{T}')
            model.simulate_robot_process(S, T, 1.0)
    ...
    def simulate_return_image(self, vel, theta, time_diff=1):
        model = self.model
        robot = model.robot
        ...
        #Symulacja Robota
        #rysowanie_pozycji robota
        log_print(f'S:{vel} T:{theta}')
        model.simulate_robot_process(vel, theta, time_diff)
        robot.calculate_led_pos()
...
if __name__ == "__main__":
    ...
    robot.calculate_led_pos()
    model = RobotBicycleModel(robot, 5)
    sim = robotSimulationEnv2LedBicycle(model)
    class cap:
        def read(self, x,y,z): return sim.simulate_return_image(x,y,z)
    
    capture = cap()
    pic1 = sim.simulate_return_image(0,0,1)
    pic2 = sim.simulate_return_image(10,0,1)
    pic3 = sim.simulate_return_image(0.1,3.14/2.0,1)
    cv.imshow('1', pic1)
    cv.imshow('2', pic2)
    cv.imshow('3', pic3)
    cv.waitKey(0)
