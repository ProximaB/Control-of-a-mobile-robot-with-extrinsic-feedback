    ...
    if(slope > 5 or slope < -5):
        for i in range(len(contours[cid])):
            ...
            if(pd1 >= 0.0 and pd2 > 0.0):
                ...
                dmax[2], M2 = update_corner(contours[cid][i], X, dmax[2], M2)
            elif(pd1 <= 0.0 and pd2 < 0.0):
                dmax[3], M3 = update_corner(contours[cid][i], Y, dmax[3], M3)
            elif(pd1 < 0 and pd2 >= 0.0):
                dmax[0], M0 = update_corner(contours[cid][i], Z, dmax[0], M0)
            else:
                continue
    else:
        halfx = (A[0]+B[0])/2
